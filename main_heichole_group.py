...
ds_root = './data/hei-chole'

from dataset.heichole_video_dataset import HeiCholeVideoDataset
...
def train (args, split_index, device, save_label):
    print(f'--Task: {args.task}, Split Index: {split_index} ...')
    if args.extractor in ['r2d', 'r2d_4layer', 'r2d_50', 'r2d_101']:
        frames_per_sample = 1
    else:
    ...
    frames_per_timestep = frames_per_sample
    ...
    if args.heatmap_regu_weight == 0:
        if args.extractor in ['r2d', 'r2d_4layer', 'r2d_50', 'r2d_101']:
            ...
            model = PartGroup_SkillNet2D(args.num_parts, args.extractor, args.context, args.aggregate, 
                                        ...
                                        final_score_bias=18, final_score_weight=7).to(device)
        else:
    ...
    video_datasets = {x: HeiCholeVideoDataset(root_dir=ds_root, is_train=(x=='train'), task=args.task, 
    ...
    if args.init_extractor:
        pretrain_dir = glob.glob(os.path.join(pt_save_root, f'SkillAcross_{args.extractor}_{args.context}_{args.aggregate}_0parts_lr*_{args.split_index}.pt'))[0]
    ...
    for epoch in range(args.num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            for samples in tqdm(dataloaders[phase]):
                inputs = samples[0].to(device)  # BxLx3x112x112 / BxLx3x4x112x112
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    ...
                    # print(outputs.shape, part_assigns.shape, part_att.shape)

                    main_loss = criterion(outputs, labels)
                    # print(f'main: {main_loss.item():.4f}')
                    loss = main_loss

                    # sconsist_loss = assign_spatialsmooth_loss(part_assigns) * 0.1
                    # print(f'sconsist: {tconsist_loss.item():.4f}')
            ...
            print(f'{phase} Loss: {epoch_loss:.4f} Rho: {epoch_rho:.4f} MAE: {epoch_l1:.4f}.')
            if phase == 'val':
...
if __name__ == '__main__':
    ...
    save_label = f"HeiChole-{args.task}_{model_type}_{args.num_parts}parts"
    if args.no_pastpro:
        save_label += "_np"
    ...
    # hist = train(args, args.split_index, device, save_label)
