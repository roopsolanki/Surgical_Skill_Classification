...
class PartGroup_SkillNet3D (nn.Module):
    def __init__ (self, num_parts=3, extractor_type='r3d', context_type='none', aggregate_type='mean',
                  avgpool_parts=False, scene_node=False, attention=False, multi_lstms=False, 
                  prepro=False, no_pastpro=False, simple_pastpro=False, 
                  final_score_weight=25, final_score_bias=15, feature_with_coord=False):
        super(PartGroup_SkillNet3D, self).__init__()
        ...
        self.feature_with_coord = feature_with_coord

        # ['stem', 'layer1', 'layer2', 'layer3', 'layer4', 'avgpool', 'fc']
        if extractor_type == 'r2p1d':
            self.extractor = models.video.r2plus1d_18(pretrained=True)
            # ['0', '1', '2', '3', '4']
            self.featmap_extractor = nn.Sequential(*list(self.extractor.children())[:-3])  # 256, 14x14
            self.num_chnals = self.featmap_extractor[-1][-1].conv2[1].num_features  # 512
        elif extractor_type == 'r2p1d_4layer':
            self.extractor = models.video.r2plus1d_18(pretrained=True)
            # ['0', '1', '2', '3', '4']
            self.featmap_extractor = nn.Sequential(*list(self.extractor.children())[:-2])  # 512, 7x7
            self.num_chnals = self.featmap_extractor[-1][-1].conv2[1].num_features  # 512
        elif extractor_type == 'r3d':
            self.extractor = models.video.r3d_18(pretrained=True)
            # ['0', '1', '2', '3', '4']
            self.featmap_extractor = nn.Sequential(*list(self.extractor.children())[:-3]) # 256, 14x14
            self.num_chnals = self.featmap_extractor[-1][-1].conv2[1].num_features  # 512
        elif extractor_type == 'r3d_4layer':
            self.extractor = models.video.r3d_18(pretrained=True)
            # ['0', '1', '2', '3', '4']
            self.featmap_extractor = nn.Sequential(*list(self.extractor.children())[:-2])  # 512, 7x7
            self.num_chnals = self.featmap_extractor[-1][-1].conv2[1].num_features  # 512
    ...
    def forward (self, clip_tensor):
        ...
        if len(clip_tensor.shape) == 5:
            tensors = clip_tensor.transpose(1,2).contiguous()   # B x CxLxHxW
            featmaps = self.featmap_extractor(tensors)  # Bx256x L/4 x14x14
            nt = featmaps.shape[2]
            featmaps = torch.cat(featmaps.unbind(dim=2), dim=0) # L'*B x 256x14x14
        elif len(clip_tensor.shape) == 6:
            _, nt, ch, short_t, h, w = clip_tensor.shape    # BxLx3x4xHxW
            assert ch == 3
            tensor = torch.cat(clip_tensor.unbind(dim=1), dim=0).contiguous()   # L*B x 3x4xHxW
            featmaps = self.featmap_extractor(tensor).squeeze(2)    # L*B x 256x14x14
        ...
        if self.context_type == 'lstm' or self.context_type == 'bilstm':
            ...
            context_out = torch.cat(torch.unbind(rnn_out, dim=3), dim=2)    # Bx L' x3*128
            context_out = context_out.transpose(1, 2).contiguous()  # Bx 3*128 xL'
            ...
            if self.scene_node: 
                ...
                scene_feat = self.scene_postpro(featmaps_mean).squeeze(3)   # L'*B x256x1
                scene_feat = torch.stack(torch.split(scene_feat, bs, dim=0), dim=1).contiguous()    # BxL'x256x1
                part_feats = torch.cat([part_feats, scene_feat], dim=3) # BxL'x256x4

            gcn_in = torch.cat(torch.unbind(part_feats, dim=1), dim=2)    # Bx256x L'*K
            gcn_in = gcn_in.transpose(1,2).contiguous()  # Bx L'*K x 256
        ...
        part_assigns = torch.stack(torch.split(part_assigns, bs, dim=0), dim=1)     # B x 4 x 3x14x14
        if not self.attention:
            part_att = torch.ones((bs, nt, self.num_parts)).to(device)   # B x 4 x 3
